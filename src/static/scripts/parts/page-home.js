...
$(document).ready(function () {
  if ($("body").hasClass("page-home")) {
    ...
    $(_showCurrentSection).text(
      $(_contentArr[0]).find(".qs_section-ttl span").text()
    );
      ...
      if (_currentIndex != undefined && _currentIndex != null) {
        if (_direction === "down") {
          if (_currentIndex != _contentArr.length - 1) {
            $(_contentArr[_currentIndex + 1]).slideDown(
              _animationTime,
              function () {
                $(_contentArr[_currentIndex]).removeClass("_current");
                if (_currentIndex + 2 <= _contentArr.length - 1) {
                  $(_showCurrentSection).text(
                    $(_contentArr[_currentIndex + 2])
                      .find(".qs_section-ttl span")
                      .text()
                  );
                } else {
        ...
        $(_contentArr[0]).slideDown(_animationTime, function () {
          ...
          $(_showCurrentSection).text(
            $(_contentArr[1]).find(".qs_section-ttl span").text()
          );
        });
      }
    }
